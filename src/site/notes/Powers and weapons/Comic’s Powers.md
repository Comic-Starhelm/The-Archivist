---
{"dg-publish":true,"permalink":"/powers-and-weapons/comic-s-powers/","dg-note-properties":{}}
---

**Greater Omnipotence**:
Cost: [ERR: No Data Available]
Description: [ERR: No Data Availabile]
**Omnipotence**:
Cost: 5 ounces of blood, memory, and item of connection
Description: allows for a steady stream of information about a given person or object for 7 hours, all knowledge of it removed after the 7 hours.
**Lesser Omnipotence**:
Cost: one ounce of blood
Description:Less information, less cost, 1 hour.
--———————————————————————-
**Healing factor**: 
Cost: exhaustion for a temporary time
Description: enhances his senses and relishes the lost blood over a rapid amount of time.
--———————————————————————-
**Soul Pact**:
Has the capacity to bind any entity to a pact, however the shaking of the hand is significantly stronger than written word, as seen by Star and Diola. Just how far can this go?
--———————————————————————-
**Conjuration**
Can summon any weapon at will, he refers to these weapons as “Soul Shields” Here’s the currently revealed ones;
“**Plastered**”
A MASSIVE rusted great sword with a razor sharp edge and the enchantment to make sure it can cut ANYTHING and it won’t heal, at least, not quickly. Supposedly there are 3 forms to this
“**Sasha**”
Supposedly named after his missing girlfriend, this elegant horse cutter katana can counter just about anything to anyone who isn’t expecting it, and one slice could very well be fatal, as it doesn’t just cut the body, it cuts your matter in all states, including your soul. the only three people who have the capacity to counter it is Star, his pupil whom he sees like a daughter, the Director, Damien, And Gabriel.

Strangely, regardless of what he is wielding, he always makes mention of how lightweight it is, despite “Plastered” being most likely hundreds of pounds of steel. We are unsure where he gets these weapons, and unsure if he has more, but the raw power is terrifying.
--——————————————————————-
**Undocumented power**
Cost:[Err: Unknown]
Description: We are unsure of the capabilities of this power, just that if he goes in, most likely it’s a killing field.
--———————————————————————-
**Future sight**
Cost [Err: Unknown]
Description: We don’t know how far, but he can definitely predict the movements of his opponents before they make them, that has been confirmed
————————————————————————-
[[Powers and weapons/Abilities hub\|Abilities hub]]